...
const migrations = [`ALTER TABLE sets
    ...
    ADD COLUMN is_public INTEGER NOT NULL DEFAULT 0`, // Tracks the original set this one was forked from (user "Edit" flow).
    // NULL means the set was created from scratch or is an original.
    `ALTER TABLE sets
        ADD COLUMN parent_slug TEXT`, // Minimum word count for the gap answer (inclusive). Default 2 matches
