<script lang="ts">
  ...
  interface DraftQuestion extends ParsedKWTQuestion { _key: number; }
...
<style>
  ...
  .top-bar   { display: flex; align-items: flex-start; justify-content: space-between; flex-wrap: wrap; gap: var(--space-4); }
  h1         { font-size: var(--font-size-3xl); font-weight: var(--font-weight-extrabold); }
  .subtitle  { color: var(--color-text-faint); font-size: var(--font-size-sm); margin-top: var(--space-1); }
  .pub-btn   { display: flex; align-items: center; gap: var(--space-2); padding: var(--space-3) var(--space-6); font-weight: var(--font-weight-semibold); flex-shrink: 0; }
  .title-row { display: flex; flex-direction: column; gap: var(--space-1); }
  .empty-state { text-align: center; color: var(--color-text-faint); padding: var(--space-8); }
  .questions { display: flex; flex-direction: column; gap: var(--space-4); }
  .add-btn   { align-self: flex-start; padding: var(--space-2) var(--space-5); }
</style>