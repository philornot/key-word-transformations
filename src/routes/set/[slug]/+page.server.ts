...
export const load: PageServerLoad = ({params}) => {
    const set = db.prepare('SELECT id, title FROM sets WHERE slug = ?').get(params.slug) as {
    ...
    const rows = db.prepare(`SELECT id, position, sentence1, sentence2_with_gap, keyword, max_words
   FROM questions WHERE set_id = ? ORDER BY position`).all(set.id) as QRow[];

    const publicSet: PublicSet = {
        id: set.id, slug: params.slug, title: set.title, questions: rows.map((r) => ({
            id: r.id,
            position: r.position,
